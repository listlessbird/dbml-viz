...
export const LINKS_ROW_PAD_BOTTOM = 6;
...
export function measureProseLineCount(
	text: string,
	bodyWidth: number,
	isValidRef: LinkValidator,
): number {
	if (text.length === 0) {
		return layout(placeholderPrepared, bodyWidth, BODY_LINE_HEIGHT).lineCount;
	}
	let total = 0;
	for (const block of text.split("\n")) {
		...
		total += measureRichInlineStats(prepared, bodyWidth).lineCount;
	}
...
export function measureLinksLineCount(
	...
	const prepared = prepareRichInline(items);
	return measureRichInlineStats(prepared, bodyWidth).lineCount;
}
